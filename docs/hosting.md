...
- <details> <summary> <b>Status</b> </summary>
    ...
    **Current decision:** no provider, migration design, or budget is selected.
    The external audio configuration has now been inspected; first recover and
    verify a working baseline before selecting hosting.
...
- <details> <summary> <b>What Must Be Hosted</b> </summary>
    ...
    - ALSA loopback capture, DarkIce MP3 encoding, and Icecast stream delivery,
      or an explicitly chosen replacement for that recovered pipeline.
    - DNS, HTTPS, secrets, supervision, logging, and recovery after reboot.

    See [architecture](architecture.md) and [Pi recovery](operations.md) for the
    known boundaries. Check Spotify's current API access, account requirements,
    and terms for the intended playback/distribution model before choosing a
    deployment design. No current service-policy or pricing research has been
    ...
    cloud-price estimate. For restoring home hosting first, see
...
- <details> <summary> <b>Options to Compare Later</b> </summary>
    ...
    | --- | --- | --- |
    | One VM for the existing stack | Closest conceptual match to a single host with local services | Can the recovered audio chain run headlessly there, and be secured and maintained? |
...
- <details> <summary> <b>Proposed Sequence</b> </summary>
    ...
    1. Recover configuration and protect the authoritative data.
    2. Reproduce one working session in an isolated environment.
    3. Confirm which historical features and rules should be preserved.
    4. Define security and dependency updates needed for public exposure.
    5. Compare complete hosting options and measured cost inputs.
    6. Test migration, data restoration, and rollback before retiring the old host.
