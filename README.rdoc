...
Start with {the documentation index}[docs/README.md] for the project overview,
architecture, letter rules, how the Raspberry Pi is operated, and what is still
outstanding.

The setup and deployment instructions below are historical notes from the
original 2020 installation, not a current installation guide. The current
start/stop/check commands are in docs/operations.md. Back up existing data and
credentials before trying setup commands on an existing installation.
